...
def post_detail(request, slug):
    post = Post.objects.fetch_with_tag_posts_count().prefetch_related('author').fetch_with_comments_count().get(slug=slug)
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all().fetch_with_tag_posts_count().prefetch_related('author').fetch_with_comments_count()[:20]
